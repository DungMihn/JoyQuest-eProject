...
const addToRecentPosts = (post: Blog) => {
  ...
  const filteredPosts = existingPosts.filter((p: Blog) => p.title !== post.title);

  // Add the post with the 'id'
  const updatedPosts = [{ id: post.id, title: post.title, imageUrl: post.imageUrl }, ...filteredPosts].slice(0, 10);
...
const D_RecentlyViewedHandler: React.FC<WrapperProps> = ({ post, children }) => {
