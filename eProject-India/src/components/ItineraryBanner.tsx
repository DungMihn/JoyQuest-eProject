...
const ItineraryBanner = () => {
  ...
  return (
    <div className="py-[60px] md:py-[120px] px-4 overflow-hidden">
      {/* Phần tiêu đề */}
      ...
      {/* Swiper */}
      <Swiper
        className=" pt-7  overflow-visible"
        modules={[Navigation]}
        navigation={{
        ...
        speed={850}
        centeredSlides={true}
        slidesPerView={1.8} // Số lượng slide hiển thị
        onSlideChange={(swiper) => setActiveIndex(swiper.realIndex)} // Cập nhật chỉ số của slide hiện tại
      >
        {/* Nút prev */}
        <div className="text-white custom-prev-button absolute left-[5%] top-1/2 transform -translate-y-1/2 cursor-pointer z-10 md:left-[15%] lg:left-[25%] hidden md:block">
          <RoundIconButton1 defaultIcon={<IoIosArrowBack />} />
        </div>

        {/* Nút next */}
        <div className="text-white custom-next-button absolute right-[5%] top-1/2 transform -translate-y-1/2 cursor-pointer z-10 md:right-[15%] lg:right-[25%] hidden md:block">
