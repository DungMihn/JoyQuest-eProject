...
const femalesActivities = activities.filter((activity) =>
  activity.categories.includes("Women Games")
);
...
const D_FemalesPage: React.FC = () => {
  ...
  const totalPages = Math.ceil(femalesActivities.length / ITEMS_PER_PAGE);
  const startIndex = (currentPage - 1) * ITEMS_PER_PAGE;
  const currentItems = femalesActivities.slice(
    startIndex,
  ...
  return (
    <div>
      ...
      <section
        ...
        style={{
          backgroundImage:
            'url("https://duruthemes.com/demo/html/bycamp/img/about.jpg")',
        }}
      >
        <div className="bg-black bg-opacity-50 w-full h-full flex flex-col justify-center items-center">
          <h3 className="text-3xl text-white mb-4">Females Promo Video</h3>
